#!/usr/bin/env python3
"""
Exposure-time sweep -- RGB888 stream.

Counterpart to led_exposure_sweep_raw.py -- same fixed frame duration,
same exposure list, same gain, same per-exposure recalibration approach,
just reading the ISP-processed RGB888 main stream instead of raw. Run
both scripts (in separate fresh processes) and compare the two CSVs to
see the genuine raw-vs-RGB888 tradeoff across exposure, with nothing
picked unevenly between the two conditions.
...
FRAME_SIZE       = (640, 400)
ANALOGUE_GAIN    = 4.0
FORCE_FRAME_DURATION_US = 6000   # MUST match led_exposure_sweep_raw.py

# IDENTICAL list to led_exposure_sweep_raw.py for a fair comparison.
EXPOSURE_SWEEP_US = [500, 1000, 1500, 2000, 3000, 4000, 5000]

SWEEP_TEST_DURATION_S = 3.0
TIMEOUT_S         = 1.0
SETTLE_S          = 0.3
OUTPUT_CSV        = "exposure_sweep_rgb.csv"
