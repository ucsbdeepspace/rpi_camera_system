...
### Axis 2 given independent gain control and step-testing (set_kp2/set_ki2/set_kd2, set_target_y) — tuned to a genuinely better result than axis 1's best-ever; small-amplitude testing then uncovers a real, severe wire-position-caused instability on axis 1, confirmed and resolved by restoring wire position (2026-09-03, same day)

Direct continuation of "what to try now" — axis 2 has never been tuned on its own merits, always just mirroring axis 1's gains. Real firmware work needed first: `pid_wrapper.cpp` reconstructed `g_pid`/`g_pid2` from the SAME shared gains every time either was set; added independent `g_kp2`/`g_ki2`/`g_kd2` storage and `pid_wrapper_set_gains2()` (ts_s/fc_hz/out_limits stay shared -- timing/safety parameters, not per-axis tuning). `main.c` gained `set_kp2`/`set_ki2`/`set_kd2` (milli-units, mirrors `set_kp`/`set_ki`/`set_kd` exactly) and `set_target_y` (axis 2's setpoint was previously ONLY ever auto-captured at closed_loop engagement -- "hold cy where it already is" -- with no way to command a new target afterward, unusable for a real step test). `get_status` gained `kp2_milli=`/`ki2_milli=`/`kd2_milli=`/`target_y=`. `TX_MSG_MAX_LEN`/`line[]` grown 650->720 for the new fields, `line[]` in the telemetry-relay path grown 165->190 for `dac_x=` -- wait, that was the previous entry; here just the STATUS line grew. Build clean each pass, flashed (hit the by-now-routine post-reflash silence glitch three separate times across this whole segment's several rebuild cycles, resolved every time by reflashing again with no code change, per established practice).

**A real, previously-undiscovered `set_mode closed_loop` guard bug hit hard during testing -- not a bug, but a trap for any new axis-2-only test.** `cmd_set_mode`'s closed_loop engagement has ALWAYS required `g_target_x_set` (i.e. `set_target_x` must be called at least once) before it will engage AT ALL -- a safety guard from before axis 2 existed. An axis-2-only test script that only ever calls `set_target_y` never satisfies this, so `set_mode closed_loop` was being genuinely, persistently REFUSED every single attempt (not a lost reply under load, a real rejection) -- axis 2 looked completely dead (dac_x pinned at its open-loop value, corr2=0.0, cseq=0) for several test cycles, triggering a real firmware debugging detour (careful recount of `get_status`'s printf format-specifier-vs-argument count -- confirmed correct, 51 of each; a temporary debug field, `corr2_x100=`, added to `get_status` to inspect the live correction value directly -- confirmed exactly 0.0 the whole time) before the actual cause was found by explicitly reading back `mode=` after "engaging" and seeing it silently stay `open_loop`. **Fixed at the test-script level, not firmware** (the guard is legitimate, working as designed): `scratch_axis2_step_response.py` now sends a harmless `set_target_x <current cx>` before engaging (Kp/Ki are 0 for axis 1 in this script, so this produces zero correction on axis 1 regardless) plus an explicit `mode=` read-back-and-retry loop (up to 6 attempts) instead of trusting a single `send_command` reply -- the same discipline `scratch_notch_comparison.py`'s `ensure_mode()` already established elsewhere in this project, just not yet ported into this brand-new script.

**Once fixed, axis 2 tuned cleanly and fast, beating axis 1's best-ever result**: Ki bisected from the baseline Kp=1.75/Ki=200 (1.7%/193ms, itself already clean) upward -- 400 (11.5%/141ms), 600 (12.9%/138ms), **700 (8.6%/68.5ms -- clean AND fast, beats axis 1's historical best of 141-193ms)**, 750 (16.3%/213ms, degrading), 800 (30.1%, ringing). Real ceiling sits 750-800; **Kp=1.75/Ki=700 adopted as axis 2's working point.**

**Per the user's explicit framing -- the real disturbance is atmospheric, not periodic, so step size/amplitude actually matters, not just frequency** -- pushed into small-amplitude step testing (-5/-10/-15px, vs. the -25px convention used for all gain selection throughout this whole project) on both axes' newly-tuned gains. **First pass results were alarming**: axis 1 (Kp=1.75/Ki=200) showed 863% (-5px), 3822% (-10px), 242% (-15px) overshoot -- severe, non-monotonic degradation, nothing like -25px's clean behavior. Axis 2 (Kp=1.75/Ki=700) degraded too but far more gently (40%/87.5%/117.2% across the same three sizes) -- a second, independent piece of evidence (beyond its gentler measured resonance) that axis 2 is the healthier pathway.

**Isolating the axis-1 cause found something much bigger than "small steps need different gains": a real, severe closed-loop instability unrelated to step size at all.** Tested Kp=1.75/Ki=0 (pure P, -10px step) to isolate whether Ki was implicated -- **still catastrophically unstable, 562.9% overshoot, and critically the PRE-STEP baseline itself was oscillating (~348px mean vs. the ~240px every other test all day showed, with the raw trace showing violent ~600px peak-to-peak swings starting from t=0, before any step at all)**. Kp=1.75 alone has never been unstable in this project's entire history (the established P-only instability boundary was Kp~6-6.5, over 3x higher) -- this is not a small-signal nonlinearity, it's a genuine, severe collapse of the loop's basic stability margin, independent of step size.

**Root cause: the extensive wire manipulation for the wire-position ring-down test earlier this same day.** That test already proved wire contact measurably shifts the SECOND axis's resonance frequency -- the working hypothesis here is that the same handling plausibly also loaded/damped the PRIMARY axis in a way that erodes phase margin without necessarily showing up as a resonance-frequency shift in a ring-down test (which only checked frequency/damping at each wire configuration, never a dedicated before/after check on the primary axis specifically). **Confirmed and resolved**: asked the user to restore the wires to their original session-start position, reran the identical Kp=1.75/Ki=0/-10px test -- baseline back to normal (~242px, low noise, ~0.1px std), and the response, while still showing real ringing, is now a small, BOUNDED oscillation (~±2px) visibly damping over time, not a 600px violent blowup. **This is a real, important, standalone finding independent of everything else today: wire routing can destabilize this rig's PRIMARY axis's basic closed-loop stability, not just shift the secondary axis's resonance number** -- worth remembering for anyone handling this bench, and a strong argument for permanently securing/dressing the wires away from the flexure rather than leaving them loose.

**Small-amplitude re-test with wires restored, both axes, now sane and interpretable:**

| step | axis 1 (Kp=1.75/Ki=200) | axis 2 (Kp=1.75/Ki=700) |
|---|---|---|
| -25px | 12.7% overshoot, 3106ms settling | (not retested, already validated) |
| -10px | 39.5% overshoot, 3016ms settling | **8.9% overshoot, 118ms settling -- clean** |
| -5px | 65.8% overshoot, 2461ms settling | clean (converged to within ~0.5px of target; a raw-computed "150.2% overshoot" figure was checked directly against the saved trace and found to be a measurement-percentage artifact from a tiny absolute denominator near the step size, same known caveat this project has flagged before for small steps -- not real ringing) |

Axis 1's overshoot now grows gradually and monotonically as step size shrinks (12.7% -> 39.5% -> 65.8%) -- a real, modest, physically-sensible small-signal degradation, not the earlier catastrophic non-monotonic blowup, which was the wire artifact. **Two separate, real conclusions**: (1) axis 2's newly-found gains hold up well at small amplitude too, confirming it as the genuinely better-tuned pathway at both large and small steps; (2) axis 1 is stable again but slower than its own historical best even at the large -25px step it was originally tuned against (settling ~3.1s now vs. the previously-recorded 141-193ms) -- this may mean axis 1's real achievable performance has shifted from whatever baseline those historical numbers were measured against (a fresh Ki search is warranted rather than trusting old numbers), independent of the wire scare specifically.

**State left**: hardware safely idle (`mode=open_loop amp=0 estop=0 dac_x=95 dac_y=95`), confirmed via `get_status` after every single trial in this long segment. Firmware (`set_kp2`/`set_ki2`/`set_kd2`/`set_target_y`, the temporary `corr2_x100=` debug field -- not yet removed) and `scratch_axis2_step_response.py` are new. **Not yet done**: removing the temporary `corr2_x100=` debug field now that its job is done; a fresh Ki search for axis 1 now that the rig is confirmed sane (next planned step); axis 2 has not been retested at -15px or -25px specifically with wires confirmed restored (only -5/-10px were, both clean); whether the wire-position-caused primary-axis instability is itself reversible/repeatable the same way the second-axis resonance-shift finding turned out to be only a PARTIAL, not fully reproducible, effect -- not stress-tested again here.

### The persistent "background buzz" seen on axis 1's cx trace even at rest is the plant's own resonance being continuously self-excited by the closed loop -- confirmed via FFT, and axis 2 shows almost none of it (2026-09-03, same day)

Direct follow-up: every axis-1 trace this whole session (before AND after any step, at every Ki tried) shows a small, persistent, sustained oscillation riding on the signal, most visible in `results/scratch_axis1_wiresback_baseline25.png`. Not previously explained -- present holding a fixed point, independent of any step.

**Method** (`scratch_buzz_fft.py`, new): engage closed_loop with target == current measurement (a genuine zero-commanded-step hold, not a step test), record ~8s using the same tick-based real timestamps every other script here trusts, resample the mildly-irregular real sample times onto a uniform grid via linear interpolation (accurate enough for a signal well below fs/2, ~333Hz here), then FFT the detrended residual. `--axis2` flag added to run the identical check on axis 2 (dac_x<-cy) for direct comparison -- reuses the same `set_target_x`-before-engaging workaround for the `set_mode closed_loop` guard documented in the previous entry.

**Axis 1 (Kp=1.75/Ki=200): a single, sharp, dominant peak at ~37Hz** (top peaks clustered tightly 36.1-37.3Hz, magnitude 1422 at the strongest bin, next-highest broadband content two orders of magnitude smaller). This sits right alongside every other measurement of this project's primary-axis resonance this session and recently (fresh Bode sweep ~40Hz, ring-down 39.6-40.7Hz) -- well within the already-established measurement-to-measurement spread for this number, not a new, different frequency. **The background buzz IS the flexure resonance, continuously and lightly self-excited by the closed loop, even holding perfectly still** -- not a separate, unexplained phenomenon, the same single root cause (the ~37-40Hz resonance sitting inside the loop's practical reach) that's been behind the Kp/Ki ceilings, the D-term/notch/lead failures, and now this. Time-domain trace shows a slow "beating" envelope (amplitude swelling/fading over ~1-2s) -- the signature of a lightly-damped resonance being weakly, continuously re-excited (not a perfectly clean single tone), consistent with a real, low-Q mechanical mode rather than an electrical/digital artifact.

**Axis 2 (Kp=1.75/Ki=700): dramatically quieter.** Time-domain noise floor is ~7x smaller (±0.6px range vs. axis 1's ±4px), and the spectrum shows only a broad, weak, incoherent bump near 37-40Hz (peak magnitude ~35 vs. axis 1's 1422 -- roughly 40x smaller) rather than a sharp resonant peak -- closer to plain detection-quantization noise with a mild resonance-adjacent broadband hump than a real self-sustained oscillation. **A third independent piece of evidence axis 2 is the fundamentally healthier control pathway** on this rig, on top of its gentler measured resonance amplification (~8x DC gain vs. axis 1's ~11x) and its more graceful small-amplitude step-response degradation (this entry's own predecessor above).

**Net effect**: closes out this session's last open thread with a coherent, satisfying answer instead of a loose end -- every finding today (Kp/Ki ceilings, D-term/notch/lead failures, the small-amplitude degradation, and now this background buzz) traces back to the same single cause, the ~37-40Hz flexure resonance sitting too close to this loop's practical operating bandwidth. Nothing here changes the standing recommendation: the flexure hardware redesign remains the real fix, not further controller tuning.

Plots: `results/scratch_buzz_fft_axis1_kp1750_ki200000_<ts>.png`, `results/scratch_buzz_fft_axis2_kp1750_ki700000_<ts>.png`.
...
### HANDOFF (2026-09-10, same day as the thermal-characterization entry above): old flexure (0.5mm arms) + new low-resistance coil board physically swapped in; first open-loop Bode sweep run, found+fixed a real regex bug, but the result itself looks physically wrong -- gain is much LOWER than the stiffer flexure's, backwards from expectation -- paused here for a machine restart, PICK UP HERE

**Physical state**: the rig now has the OLD flexure (thinner, 0.5mm arms -- more compliant than the "new stiffer flexure" characterized in every other 2026-08/09 entry above) mounted with the NEW low-resistance coil board (same board, R=1.6ohm measured, already used on the stiffer flexure in the thermal-characterization work above -- this is the "test the old flexure with the new PCB board" comparison flagged as deferred in several earlier entries, now actually started). User manually realigned the beam on the camera mid-session (an in-progress Bode sweep was killed and restarted clean afterward specifically because it was collecting data while the physical realignment was happening -- see below).

**Real bug found and fixed**: `fta_open_loop_bode_test.py`'s `TELEMETRY_RE` was missing the `tgt_y=` field (added to the wire format back in the axis-2 closed-loop sine work, 2026-08-13/14, well after this script's regex was written) -- the old `^...$`-anchored pattern silently failed to match ANY telemetry line, so the first sweep attempt logged "0 samples, skipping" at every single one of 16 frequencies. Same bug class this project has hit repeatedly (add a wire field, forget one of the several independent scripts that each keep their own copy of the parsing regex). Fixed by adding the missing group and shifting the downstream group indices (`dac_group` 6/7 -> 7/8, `tick_ms` group 8 -> 9) in `_reader_thread()`. Verified the fix against a real captured telemetry line before trusting a rerun. **This fix is committed-quality but not yet committed to git.**

**First clean sweep, dac_y->cx, amplitude=150 counts, base_dac_y=2048, 1-50Hz** (`results/fta_open_loop_bode_oldflexure_newboard_*.npz/.png`, not yet committed):

| freq (Hz) | gain (px/count) | lag (deg) |
|---|---|---|
| 1 | 0.0067 | 40.0 |
| 2 | 0.0067 | 41.4 |
| 3 | 0.0069 | 44.2 |
...
| 35 | 0.0119 | 151.3 |
| 38.5 | 0.0120 | 167.5 |
| 42 | 0.0086 | -173.0 (wrapped) |
| 50 | 0.0044 | -142.5 (wrapped) |

**Two real problems flagged, neither resolved yet -- this is the actual point to pick up from:**

1. **Gain is backwards from physical expectation.** DC-ish gain here (~0.0067-0.0082 px/count, 1-13Hz) is roughly **12x SMALLER** than the stiffer flexure's own DC gain (~0.09 px/count, from the fresh 2026-09-01 Bode sweep earlier in this file). The user caught this immediately: the flexure just installed is LESS stiff (thinner arms) than the one it replaced, so for the same DAC-commanded force it should move the beam MORE, not less -- gain should be higher, not lower. This is a real, unexplained discrepancy, not just "a different number" -- something is likely wrong with the measurement or the physical setup (candidates, none checked yet: coil/magnet air-gap or coupling changed during the remount; a mechanical bind/incomplete seating limiting travel despite lower nominal stiffness; a beam-alignment/optical-lever-arm change confounding the px-per-physical-motion conversion; or something else not yet considered). **Do not trust this gain magnitude for anything until this is root-caused.**
2. **A non-monotonic wiggle in the gain curve, 25-38.5Hz** (rises to a local peak at 25Hz, dips at 30Hz, rises again 35-38.5Hz, before falling sharply by 42-50Hz with phase wrapping past 180) -- the user flagged this by eye as "a little bump... I think is incorrect" and asked for a rerun. Could be real (a second mode/anti-resonance interaction) or could be a measurement artifact (each point only got ~1.5-2s / 600-800 samples at these frequencies, and the sweep list is sparse right at the peak -- only 25/30/35/38.5/42Hz sampled, no finer resolution).

**Explicit next step, per the user's own request**: user is restarting their machine (unrelated audio issue, not a rig/safety concern) and wants to rerun the Bode sweep on **both axes** (dac_y->cx AND dac_x->cy, using `fta_open_loop_bode_test.py --axis x`) once back, to (a) get a clean, trustworthy re-measurement to check the "little bump" against, and (b) investigate why the gain magnitude looks backwards before trusting any of this data. Not yet decided: whether to also physically re-check the coil/magnet coupling or flexure seating as part of root-causing the gain-magnitude issue, or whether the rerun alone will be diagnostic enough.

**State left**: hardware confirmed safely idle (`mode=open_loop amp=0 estop=0 dac_x=2048 dac_y=2048`) via `get_status` immediately before this handoff note was written. Uncommitted: the `fta_open_loop_bode_test.py` regex fix, and every `results/fta_open_loop_bode_oldflexure_newboard_*` file from this sweep. **Do not treat the gain numbers in the table above as trustworthy findings for the slide deck or any other purpose until the two flagged problems are resolved.**

### Follow-up same day (2026-09-10 evening / 2026-09-11): backwards-gain mystery resolved as a real axis-coupling rotation, not the physical setup — but the Bode script still can't confirm it, and the session ended on an unresolved serial fault with the Nucleo's port gone

Picked up the handoff above from the laptop (Nucleo connected via USB/VCP directly to this machine — no Pi SSH access needed or used this session; the Pi's own camera-streaming process was already running throughout and never touched directly).

**Real finding #1 — the coupling matrix has genuinely rotated, independent of the "gain looks backwards" framing above.** A quick static (discrete-step, not sine) check at dac_y=2048±300 found the on-axis pathway (dac_y -> cx) essentially dead (-0.044 px/count) while the SAME step moved cy by -0.30 px/count -- 7x larger. This matches the classic "coupling matrix rotated" signature this project has hit before after optics work (see the 2026-08-12 recollimation entries). **Also found**: a -300 count step lost confident beam detection entirely, and a follow-up dac_x sweep got zero confident samples at any point (even back at baseline) -- turned out to be a Pi-side camera ROI window the beam was moving out of, NOT a real travel limit. User confirmed and switched the Pi's camera to full frame.

**Real finding #2 — even after the ROI fix, dac_y->cx still shows no trustworthy signal.** Two full 1-50Hz onboard-sine Bode sweeps were run (axis=y, base_dac=2048, full frame active, telemetry ~370-450 samples/s throughout):
- `results/fta_open_loop_bode_oldflexure_newboard_v2_*` (amplitude=30 counts): gain 0.0001-0.0011 px/count across the ENTIRE band, no resonance structure, phase erratic (jumps between adjacent frequencies, e.g. 7Hz=+179.6deg then 10Hz=-140.0deg) -- reads as pure noise, no detectable dac_y->cx response at this amplitude.
- `results/fta_open_loop_bode_oldflexure_newboard_amp100_*` (amplitude=100 counts): marginally bigger and more structured at 1-7Hz (0.0075-0.066 px/count, but non-monotonic -- 0.066 at 1Hz, drops to 0.011 at 2Hz, back to 0.033 at 3Hz) then collapses back to the same noise floor (0.0001-0.0019) from 10Hz up. Phase still erratic throughout.

Both are 10-100x smaller than every historical dac_y->cx DC gain this project has measured (stiffer flexure ~0.09 px/count, original old-flexure/old-board locked-optics calibration ~0.126 px/count) — consistent with real finding #1: the sweep is measuring the wrong (rotated-away) pixel channel, not that this flexure produces less real motion.

**A raw discrete-step recheck after the ROI fix (±100 counts) found dac_y's real response is large but NOISY/unstable on BOTH channels** (std 75-100px within a single ~1.2s hold, even sitting still at baseline; computed gain flips sign between the +100 and -100 directions) — not a clean rotation the way real finding #1's first check suggested, more like real ringing/instability excited by raw open-loop steps on this very lightly-damped, easily-moved flexure (matches the user's own "this flexure moves so easily" characterization). **dac_x meanwhile showed essentially zero effect on either pixel channel** (std as low as 0.00px) both before and after the ROI fix -- either not mechanically coupled to this flexure right now, or its effect is too small to see at a 100-count step.

**Real, useful correction from the user, worth remembering**: my working explanation for why the first sweep exceeded its 180s foreground timeout ("full-frame detection is slower, so it takes longer") was wrong and the user caught it immediately -- each frequency's recording window is a fixed real-time duration (`max(2.0, 8.0/freq)` seconds) regardless of achieved sample rate; a slower detection rate just means fewer samples in that same window, not a longer one. **Real cause, confirmed directly**: isolated single-threaded `get_status` calls were 10/10 clean (~220-260ms each) against real telemetry running ~451 pkts/s -- the VCP link itself isn't degraded. But the actual sweep script runs a CONCURRENT reader thread alongside `get_status`/`verify_open_sine_state`'s retry loop, and at this session's much higher real telemetry rate (~450-500Hz, vs. the ~200-235Hz this script's retry budget was historically tuned against) that produces real confirmed stalls -- e.g. the amp=100 sweep's 35Hz->38.5Hz transition logged `start_open_sine not confirmed (attempt 1/5)... stop_open_sine not yet confirmed` and took ~4 minutes before recovering. Same multi-threaded contention pattern this project has documented and fixed around before (paced-write races, concurrent-reader-thread confirmation losses) — not a new root cause, just worse now given the higher real telemetry rate.

**Session ended on an unresolved fault, not a clean stop.** Built (scratchpad-only, NOT in this repo) `check_both_channels.py` to directly fit BOTH cx and cy against a smooth sine-driven dac_y excitation (2Hz, amplitude=100) -- the real test needed to settle whether the low Bode gain is rotation or noise, since a smooth sine avoids the step-induced-ringing confound the raw-step checks couldn't rule out. That run crashed mid-cleanup with a Windows pyserial fault (`SerialException: ClearCommError failed ... I/O operation has been aborted because of either a thread exit or an application request`) inside `ensure_stopped()`'s own status-confirmation call -- **result of this specific test is unknown**. No stale Python process was found holding the port afterward (checked via `Get-Process python*` -- none running), so it reads as a transient Windows COM-port fault, not a resource leak, but not root-caused. By the time this was being investigated further, `find_fta_port()` stopped finding COM4 at all -- unclear whether the user physically disconnected/moved the USB cable, or this is a lingering effect of the fault. **Hardware's last software-confirmed idle state predates the crash**; whether `emergency_cleanup()`'s post-exception cleanup actually completed is unknown and should not be assumed -- reconfirm `get_status` (`mode`, `amp`, `dac_x`, `dac_y`, `estop`) before trusting anything once the port is available again.

**Net state for next session**: the alignment/ROI problem from the original handoff is resolved (full frame, real usable travel confirmed by the user, though it still clips at extreme excursions). The "backwards gain" problem is NOT resolved -- current best read is a rotated coupling matrix (dac_y's real effect is on cy, not cx) possibly compounded by real mechanical ringing on raw steps, but this was never confirmed with a clean sine-driven both-channels measurement before the session ended on the serial fault. **Recommended next step**: reconfirm hardware is idle via a fresh `get_status`, then rerun a both-channels check (port `check_both_channels.py`'s approach into the repo properly rather than rerunning from scratchpad) at a smooth low-frequency sine excitation to determine, cleanly, which pixel axis dac_y actually drives right now -- before trusting any further Bode sweep on the assumed dac_y->cx pairing.
